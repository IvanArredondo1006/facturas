...
st.write("Sube una carpeta comprimida (.zip) que contenga tus facturas en PDF. El sistema aplicará OCR para extraer: **proveedor/productor, NIT, fecha de emisión y total a pagar**.")

uploaded_zip = st.file_uploader("Sube un archivo ZIP con PDFs", type="zip")

# 🧠 Detectar nuevo ZIP y resetear resultados previos
if uploaded_zip:
...
# 📄 Función robusta de extracción de campos
def extraer_campos_factura(texto):
    ...
    nit = (
        ...
        re.search(r'Document[o|o Nacional| No]*[:\-]?\s*([0-9\-\.]{7,20})', texto, re.IGNORECASE) or
        re.search(r'\b(\d{7,14}[-]?\d?)\b', texto)
    )

    fecha = (
        re.search(r'Fecha\s+de\s+Liquidaci[oó]n[:\-]?\s*(\d{1,2}\s*/\s*\d{1,2}\s*/\s*\d{4})', texto, re.IGNORECASE) or
        re.search(r'Fecha\s*[:\-]?\s*(\d{1,2}/\d{1,2}/\d{4})', texto) or
        re.search(r'Date\s*[:\-]?\s*(\d{1,2}/\d{1,2}/\d{4})', texto, re.IGNORECASE)
    )

    valor_total = (
        re.search(r'Total\s+a\s+Pagar\s*[:\-]?\s*([0-9\.,]+)', texto, re.IGNORECASE) or
        re.search(r'Valor\s+Total\s+(?:CPT|EXW|FOB)?\s*[:\-]?\s*([0-9\.,]+)', texto, re.IGNORECASE) or
        re.search(r'Total\s+Amount\s*[:\-]?\s*([0-9\.,]+)', texto, re.IGNORECASE) or
        re.search(r'Montant\s+Total\s*[:\-]?\s*([0-9\.,]+)', texto, re.IGNORECASE)
    )
...
# ⚙️ Procesamiento del ZIP si no se ha hecho antes
if uploaded_zip and "df_resultado" not in st.session_state:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i, ruta_pdf in enumerate(pdf_files):
            ...
            try:
                imagenes = convert_from_path(ruta_pdf, poppler_path=r"C:\Program Files (x86)\poppler-24.08.0\Library\bin", first_page=1, last_page=1)
                pytesseract.pytesseract.tesseract_cmd = r'C:\Users\arredondoivan\AppData\Local\Programs\Tesseract-OCR\tesseract'
